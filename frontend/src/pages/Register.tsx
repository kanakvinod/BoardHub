...
import { User, Lock, Mail, Loader2, ClipboardList } from "lucide-react";
...
export const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error?.message || "Something went wrong. Please check details."
      );
    } finally {
      setIsSubmitting(false);
    }
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Get started for free and manage your project tasks
          </p>
        </div>

        <div className="glass-panel p-8 rounded-3xl shadow-2xl border border-slate-800">
          <h2 className="text-xl font-semibold text-slate-200 mb-6">Sign Up</h2>
